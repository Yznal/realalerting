...
class RateReporter {
public:
  typedef std::function<void(double, double, std::int64_t, std::int64_t)>
      on_rate_report_t;

  RateReporter(nanoseconds reportInterval, const on_rate_report_t &onReport)
      : m_reportInterval(reportInterval), m_onReport(onReport),
  ...
  void report() {
    std::int64_t totalBytes =
        std::atomic_load_explicit(&m_totalBytes, std::memory_order_acquire);
    std::int64_t totalMessages =
        std::atomic_load_explicit(&m_totalMessages, std::memory_order_acquire);
    steady_clock::time_point timestamp = steady_clock::now();

    const double timeSpanSec =
        duration<double, std::ratio<1, 1>>(timestamp - m_lastTimestamp).count();
    const double messagesPerSec =
        static_cast<double>(totalMessages - m_lastTotalMessages) / timeSpanSec;
    const double bytesPerSec =
        static_cast<double>(totalBytes - m_lastTotalBytes) / timeSpanSec;
  ...
  void reset() {
    std::int64_t currentTotalBytes =
        std::atomic_load_explicit(&m_totalBytes, std::memory_order_relaxed);
    std::int64_t currentTotalMessages =
        std::atomic_load_explicit(&m_totalMessages, std::memory_order_relaxed);
  ...
  inline void halt() { m_running = false; }
  ...
  inline void onMessage(std::int64_t messages, std::int64_t bytes) {
    ...
    std::atomic_store_explicit(&m_totalBytes, totalBytes + bytes,
                               std::memory_order_release);
    std::atomic_store_explicit(&m_totalMessages, totalMessages + messages,
                               std::memory_order_release);
  }
